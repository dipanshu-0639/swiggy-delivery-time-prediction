...
def drop_missing_values(data: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"The dataset with missing value dropped has {df_dropped.shape[0]} rows and {df_dropped.shape[1]} columns")
...
def perform__transformations(preprocessor,data:pd.DataFrame):
    #transform the data
...
if __name__=="__main__":
    ...
    #load the train and tes data with missing values dropped
    ...
    #transform the data
    X_train_trans=perform__transformations(preprocessor=preprocessor,data=X_train)
    logger.info("Train data is transformed.")
    X_test_trans=perform__transformations(preprocessor=preprocessor,data=X_test)
